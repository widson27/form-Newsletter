  ...
  return (
    <form className="flex flex-col gap-3" onSubmit={handleSubmit}>
      <div className=" flex flex-col">
        ...
        <input className="rounded-lg py-2 px 2 text-sm placeholder:text-sm placeholder:text-stone-400" type="text" placeholder="Digite seu nome" value={name} onChange={(e) => setName(e.target.value)} />
        {errors?.name && <small className="text-xs text-red-500 mt-1  " >{errors?.name}</small>}
      </div>
      <div className=" flex flex-col">
        <label className="text-sm" htmlFor="email">E-mail</label>
        <input className="rounded-lg py-2 px 2 text-sm placeholder:text-sm placeholder:text-stone-400" type="email" placeholder="Insira seu email" value={email} onChange={(e) => setEmail(e.target.value)} />
